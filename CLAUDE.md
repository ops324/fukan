...
- **commit / push は明示依頼があったときだけ**行う。リスクある変更は作業ブランチでローカル検証してから main へ。
- **自動ジョブとの共存**: `scripts/auto-generate.sh` が差分を自動 commit/push する。`src/` や `templates/` に**作業途中（dirty）の変更を放置しない**——commit するか stash／退避してから離席する。
