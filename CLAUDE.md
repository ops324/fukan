...
- **コード改善・機能追加・リスクある変更は作業ブランチで行う（既定）**。`main` 直コミットは記事内容や軽微な文言／ドキュメント微修正に限る。
  ...
  - 自動ジョブがコミットするのは **`data/` だけ**（`git add -- data`）。生成物は `dist/`（gitignore）に出るため commit されない。よって `src/` のコード変更とは通常競合しない（マージ/リベースはクリーンになりやすい）。
    **`assets/` は自動コミットされない**——手動登録したプレス画像（`/assets/press/...`）を置いたままにすると、`articles.json` は参照しているのに実体が push されず**本番で 404** になる。ジョブは検出して通知するので、自分で commit すること。
