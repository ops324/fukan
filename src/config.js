...
export const config = {
  ...
  qualityThresholds: {
    ...
    // --- 二重掲載の取り込みブロック ---
    // 背景: 同じ Starship 試験飛行を出典違いで2本公開しトップに並べた（2026-07-25）。link が違うと
    // link 重複排除は効かず、類似度も警告止まりで ingest を通過していた。
    // 実測でこのケースの Jaccard は 0.278（警告の 0.6 にも届かない）— 表記揺れ（SpaceX/スペックス）で
    // 日本語 bigram の共通部分が消えるため。一方 **特徴語（英数字）の包含率は 0.83** で明確に出た。
    // よって主たる防壁は包含率にし、Jaccard 側は「見出しがほぼ同一」のケース用に高い値で併置する。
    dupBlockJaccard: 0.78,
    dupBlockContainment: 0.8, // 特徴語の包含率がこれ以上なら二重掲載として取り込まない
    dupBlockMinShared: 3,     // かつ共通する特徴語がこれ以上（1〜2語での誤ブロックを防ぐ）
  ...
  // --- 下書きの決定論リント（draftLint）---
  // veto の最多カテゴリ（numeric）への対策が「プロンプトの文言」しか無かったため、writer が
  // 自己批評で**実際に実行する**検算器を置く（src/lintDrafts.js）。出典を読まずに分かる矛盾
  // ——要約層だけが主張する数値／同一記事内の比率の食い違い／為替換算の疑い／全角合成文字／
  // 同じ回の別記事からの語の混入——を拾う。出典との照合は judge の仕事であり代替しない。
  // すべて警告（公開はブロックしない）。偽陽性は前提で、writer/judge には「確認の起点」と伝える。
  draftLint: {
    enabled: true,          // 異常時はここを false にすれば素通りする（lintDrafts が空配列を返す）
    crosstalkMaxDrafts: 2,  // この本数以下の下書きにしか出ない語だけを「稀語」として混入判定に使う
    corpusRecent: 300,      // 常出語の判定に使う直近の公開記事数（文書頻度の母集団）
    corpusDfMax: 3,         // 直近記事のうちこの本数以上に出る語は「サイトの常出語」＝混入判定から外す
                            // 実測(2026-07-26): 3 で データセンター/アクセス/Nvidia/OpenAI が外れ、
                            // 実際の混入語（Brent df=0・アラムコ df=0）は残る。指摘は 81→17 件に減る。
    maxFindingsPerDraft: 6, // 1本あたりの提示上限（指摘が多すぎると重要な1件が埋もれる）
  },
  ...
  // 自動ジョブ（launchd・6時/18時）が認証切れで無言停止し、6ラン連続で記事ゼロのまま
  // 3日気づけない事故が起きた（2026-07-22〜25）。articles.json は壊れていないので他の検査は
